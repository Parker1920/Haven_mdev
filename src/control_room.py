...
class ControlRoom(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        if not self._frozen:
            ...
            # Note: JSON Import removed - discoveries now come directly from Discord bot via VH-Database.db
...
between JSON and database backends.
    ...
    # DEPRECATED: JSON import removed - discoveries now come directly from Discord bot
    def show_import_json_dialog(self):
        """Show JSON import dialog (Phase 5) - DEPRECATED - Discovery imports now happen via Discord bot"""
        messagebox.showinfo(
            "Feature Removed",
            "JSON discovery imports have been removed.\n\n"
            "Discoveries are now submitted via The Keeper Discord bot using /discovery-report,\n"
            "which writes directly to VH-Database.db.\n\n"
            "Use /haven-export in Discord to create backup JSON files if needed."
        )
        # Old implementation has been removed - see git history if needed
