...
class TestCoordinateValidation:
    """Test coordinate validation with pytest."""

    @pytest.mark.unit
    def test_valid_coordinates(self):
        """Test valid coordinates pass validation."""
        is_valid, error = validate_coordinates(10.5, 20.3, 5.1)
        assert is_valid is True
        assert error == ""

    @pytest.mark.unit
    def test_boundary_coordinates_min(self):
        """Test minimum boundary coordinates."""
        is_valid, error = validate_coordinates(-100, -100, -25)
        assert is_valid is True

    @pytest.mark.unit
    def test_boundary_coordinates_max(self):
        """Test maximum boundary coordinates."""
        is_valid, error = validate_coordinates(100, 100, 25)
        assert is_valid is True

    @pytest.mark.unit
    def test_x_coordinate_out_of_range(self):
        """Test X coordinate exceeds upper bound."""
    ...
    def test_y_coordinate_out_of_range(self):
        """Test Y coordinate exceeds upper bound."""
        is_valid, error = validate_coordinates(0, 150, 0)
        assert is_valid is False
        assert "Y coordinate" in error

    @pytest.mark.unit
    def test_z_coordinate_out_of_range(self):
        """Test Z coordinate exceeds upper bound."""
        is_valid, error = validate_coordinates(0, 0, 50)
        assert is_valid is False
        assert "Z coordinate" in error

    @pytest.mark.unit
    def test_coordinate_type_error_x(self):
        """Test X coordinate with wrong type."""
        is_valid, error = validate_coordinates("10", 20, 5)  # type: ignore
        assert is_valid is False
        assert "must be a number" in error


class TestSystemNameValidation:
    """Test system name validation with pytest."""
    ...
    def test_empty_system_name(self):
        """Test empty system name is rejected."""
        is_valid, error = validate_system_name("")
        assert is_valid is False
        assert "cannot be empty" in error

    @pytest.mark.unit
    def test_system_name_too_long(self):
        """Test system name exceeding 100 character limit."""
        long_name = "A" * 150
        is_valid, error = validate_system_name(long_name)
        assert is_valid is False
        assert "too long" in error

    @pytest.mark.unit
    def test_system_name_invalid_type(self):
        """Test non-string system name."""
        is_valid, error = validate_system_name(123)  # type: ignore
        assert is_valid is False
        assert "must be a string" in error


class TestGenerationReportStats:
    """Test validation report generation with mocking."""

    @pytest.mark.unit
    def test_report_structure(self, sample_data_file):
        """Test report has correct structure."""
        report = generate_validation_report(sample_data_file)
    ...
    @pytest.mark.unit
    def test_report_stats_contain_counts(self, sample_data_file):
        """Test report stats contain all required counters."""
        report = generate_validation_report(sample_data_file)
        stats = report["stats"]
        
        assert "total_systems" in stats
        assert "total_planets" in stats
        assert "total_moons" in stats
    ...
    def test_report_empty_data(self):
        ...
        report = generate_validation_report(empty_data)
        
        assert report["stats"]["total_systems"] == 0
        assert isinstance(report["stats"]["regions"], list)

    @pytest.mark.unit
    def test_report_with_multiple_systems(self):
        """Test report correctly counts multiple systems."""
        data = {
            "_meta": {"version": "3.0.0"},
            "System1": {"id": "S1", "name": "S1", "region": "R1", "x": 0, "y": 0, "z": 0, "planets": []},
            "System2": {"id": "S2", "name": "S2", "region": "R1", "x": 1, "y": 1, "z": 1, "planets": []},
            "System3": {"id": "S3", "name": "S3", "region": "R2", "x": 2, "y": 2, "z": 2, "planets": []},
        }
        report = generate_validation_report(data)
        
        assert report["stats"]["total_systems"] == 3


class TestValidateDataFile:
    """Test complete data file validation."""

    @pytest.mark.unit
    def test_validate_valid_data_file(self, sample_data_file):
        """Test validation of well-formed data file."""
        is_valid, errors = validate_data_file(sample_data_file)
        # Should validate successfully
        assert isinstance(is_valid, bool)
        assert isinstance(errors, list)

    @pytest.mark.unit
    def test_validate_empty_data_file(self):
        """Test validation of empty data file."""
        empty_data: Dict[str, Any] = {}
        is_valid, errors = validate_data_file(empty_data)
        assert isinstance(is_valid, bool)


# Import at module level for Dict and Any types
