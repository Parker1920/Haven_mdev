...
try:
    # JSON backend was deprecated; ensure Database provider is available
    ...
    # Optional check: importing JSONDataProvider should succeed if JSON mode is available
    try:
        from src.common.data_provider import JSONDataProvider
        try:
            json_provider = JSONDataProvider()
            print(f"  ✓ JSONDataProvider initialized")
        except Exception as e:
