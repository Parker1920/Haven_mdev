...
function drawCompass(angleRad){
    ...
    ctx.rotate(angleRad);
    ctx.strokeStyle = '#00C2B3'; ctx.lineWidth = 2;
    ...
    ctx.restore();
    ctx.fillStyle = '#00C2B3'; ctx.font = '10px Rajdhani, sans-serif'; ctx.textAlign='center';
...
const fillLight = new THREE.DirectionalLight(0x00C2B3, 0.3);
...
scene.add(starField);
    ...
    const majorGrid = new THREE.GridHelper(200, 16, 0x00C2B3, 0x00C2B3);
...
SYSTEM_DATA.forEach(item => {
    ...
    if (false && config.showLabel && VIEW_MODE === 'galaxy' && itemType === 'system') {
        ...
        context.fillStyle = 'rgba(0, 194, 179, 1)';
...
if (VIEW_MODE === 'system') {
    ...
    radii.forEach(r => {
        ...
        const ringMat = new THREE.MeshBasicMaterial({ color: 0x00C2B3, side: THREE.DoubleSide, transparent: true, opacity: 0.15 });
...
renderer.domElement.addEventListener('click', (e) => {
    ...
    if ((ud.type === 'planet' || ud.type === 'moon') && window.DISCOVERIES_DATA && window.DISCOVERIES_DATA.length > 0) {
        ...
        if (locationDiscoveries && locationDiscoveries.length > 0) {
            html += `<p style="margin-top: 12px;"><button id="view-discoveries-btn" style="background: #00C2B3; color: #000; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;">📍 View ${locationDiscoveries.length} Discover${locationDiscoveries.length !== 1 ? 'ies' : 'y'}</button></p>`;
        }
    }
    ...
    if (data.photo) {
        ...
        if (href) {
            html += `<p><strong>Photo:</strong> <a href="${href}" target="_blank" style="color: #00C2B3;">View</a></p>`;
        }
    }
        ...
        discBtn.addEventListener('click', () => {
            ...
            if (locationDiscoveries && locationDiscoveries.length > 0) {
                ...
                const renderDiscoveries = (filterType = 'All') => {
                    ...
                    discoveryHtml += `<label style="color: #00C2B3; font-weight: bold; margin-right: 8px;">Filter:</label>`;
                    discoveryHtml += `<select id="discovery-filter" style="background: #222; color: #fff; border: 1px solid #00C2B3; padding: 4px 8px; border-radius: 4px; cursor: pointer;">`;
                    ...
                    if (filtered.length === 0) {
                        ...
                        filtered.forEach((disc, idx) => {
                            discoveryHtml += `<div style="background: rgba(0, 194, 179, 0.08); border-left: 3px solid #00C2B3; padding: 12px; margin: 10px 0; border-radius: 4px;">`;
                            ...
                            discoveryHtml += `<p style="margin: 0 0 8px 0;"><strong style="color: #00C2B3;">${typeEmoji} ${typeName}</strong>${tierBadge}</p>`;
                            ...
                            if (fields.length > 0) {
                                ...
                                fields.forEach(field => {
                                    ...
                                    if (value) {
                                        ...
                                        detailsHtml += `<div style="margin: 4px 0;"><span style="color: #00C2B3; font-size: 12px; font-weight: bold;">${label}:</span> <span style="color: #ddd; font-size: 13px;">${value}</span></div>`;
                                    }
                                });
                                if (detailsHtml) {
                            ...
                            if (photoUrl) {
                                discoveryHtml += `<div style="margin: 12px 0;">`;
                                discoveryHtml += `<p style="margin: 0 0 6px 0; color: #00C2B3; font-weight: bold; font-size: 13px;">📸 Evidence Photo:</p>`;
                                discoveryHtml += `<a href="${photoUrl}" target="_blank" style="display: block;">`;
                                discoveryHtml += `<img src="${photoUrl}" style="max-width: 100%; max-height: 300px; border: 2px solid #00C2B3; border-radius: 6px; cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">`;
                                discoveryHtml += `</a>`;
                            ...
                            if (disc.coordinates) {
                                discoveryHtml += `<p style="margin: 6px 0; font-size: 13px;"><span style="color: #00C2B3; font-weight: bold;">📍 Coordinates:</span> <code style="background: rgba(0, 194, 179, 0.15); padding: 2px 6px; border-radius: 3px; font-family: monospace; color: #00C2B3;">${disc.coordinates}</code></p>`;
                            }
                            ...
                            if (disc.time_period && disc.time_period !== 'Unknown Era') {
                                discoveryHtml += `<p style="margin: 6px 0; font-size: 13px;"><span style="color: #00C2B3; font-weight: bold;">🕐 Temporal Marker:</span> ${disc.time_period}</p>`;
                            }
                            if (disc.condition && disc.condition !== 'Indeterminate') {
                                discoveryHtml += `<p style="margin: 6px 0; font-size: 13px;"><span style="color: #00C2B3; font-weight: bold;">⚡ Signal Strength:</span> ${disc.condition}</p>`;
                            }
