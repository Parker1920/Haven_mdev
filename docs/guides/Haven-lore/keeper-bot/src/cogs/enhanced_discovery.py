...
class EnhancedDiscoverySystem(commands.Cog):
    ...
    async def process_discovery_submission(self, interaction: discord.Interaction, discovery_data: dict):
        ...
        try:
            ...
            # Add to sync queue for automatic sync to VH-Database
            try:
                sync_queue = self.bot.sync_worker.sync_queue
                await sync_queue.add_to_queue(discovery_id, metadata={
                    'system_name': enhanced_data.get('system_name'),
                    'location_type': enhanced_data.get('location_type'),
                    'discovery_type': enhanced_data.get('type')
                })
                logger.info(f"Discovery {discovery_id} queued for sync to VH-Database")
            except Exception as e:
                logger.warning(f"Could not add to sync queue: {e}")
